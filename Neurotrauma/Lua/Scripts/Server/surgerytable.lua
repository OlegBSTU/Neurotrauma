...
local function GetHeartrate(character)
    ...
    if (fibrillation ~= nil) then
        rate = HF.Lerp(MaxTachycardiaHeartrate, MaxFibrillationHeartrate,
...
Hook.Add("surgerytable.update", "surgerytable.update", function(effect, deltaTime, item, targets, worldPosition)
    ...
    if target.IsDead or HF.HasAffliction(target, "sym_unconsciousness", 0.1) then item.SendSignal("0", "conscious_out") else
        item.SendSignal("1", "conscious_out") end
