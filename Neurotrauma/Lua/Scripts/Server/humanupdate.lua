...
NT.Afflictions = {
    ...
    ra_fracture = {
        update = function(c, i)
            if c.afflictions[i].strength > 0 then
                c.afflictions[i].strength = c.afflictions[i].strength +
                2 * HF.BoolToNum(not HF.HasAfflictionLimb(c.character, "gypsumcast", LimbType.RightArm)) * NT.Deltatime
            end
        end
    },
    ll_fracture = {
    ...
    rl_fracture = {
        update = function(c, i)
            if c.afflictions[i].strength > 0 then
                c.afflictions[i].strength = c.afflictions[i].strength +
                2 * HF.BoolToNum(not HF.HasAfflictionLimb(c.character, "gypsumcast", LimbType.RightLeg)) * NT.Deltatime
            end
        end
    },
    n_fracture = {
    ...
    heartdamage = { update = function(c, i)
        if c.stats.stasis then return end
        c.afflictions[i].strength = NT.organDamageCalc(c,
    ...
    lungdamage = { update = function(c, i)
        if c.stats.stasis then return end
        c.afflictions[i].strength = NT.organDamageCalc(c,
    ...
    kidneydamage = {
        update = function(c, i)
            if c.stats.stasis then return end
            c.afflictions[i].strength = kidneyDamageCalc(c,
    ...
    bonedamage = {
        update = function(c, i)
            ...
            c.afflictions[i].strength = NT.organDamageCalc(c,
                ...
                (c.afflictions.sepsis.strength / 500 + c.afflictions.hypoxemia.strength / 1000 + math.max(c.afflictions.radiationsickness.strength - 25, 0) / 600) *
                NT.Deltatime)
            ...
            if (c.afflictions.kidneydamage.strength > 70) then c.afflictions[i].strength = c.afflictions[i].strength +
                (c.afflictions.kidneydamage.strength - 70) / 30 * 0.15 * NT.Deltatime end
        end
    },
    organdamage = { max = 200, update = function(c, i)
        if c.stats.stasis then return end
        c.afflictions[i].strength = c.afflictions[i].strength + c.stats.neworgandamage -
        0.03 * c.stats.healingrate * NT.Deltatime
    end },
    -- Blood
    sepsis = {
        update = function(c, i)
            if (c.afflictions.afantibiotics.strength > 0.1) then c.afflictions[i].strength = c.afflictions[i].strength -
                NT.Deltatime end
            if c.stats.stasis then return end
            if (c.afflictions[i].strength > 0.1) then c.afflictions[i].strength = c.afflictions[i].strength +
                0.05 * NT.Deltatime end
        end
    },
    immunity = {
        default = -1,
        ...
        update = function(c, i)
            ...
            c.afflictions[i].strength = HF.Clamp(
            c.afflictions[i].strength + (0.5 + c.afflictions[i].strength / 100) * NT.Deltatime, 5, 100)
        end
    },
    bloodloss = { max = 200 },
    bloodpressure = {
        ...
        update = function(c, i)
            ...
            if (desiredbloodpressure > c.afflictions.bloodpressure.strength) then bloodpressurelerp = bloodpressurelerp /
                3 end
    ...
    hypoxemia = {
        update = function(c, i)
            ...
            if regularHypoxemiaChange > 0 then
                ...
                regularHypoxemiaChange = HF.Lerp(regularHypoxemiaChange * 2, 0, HF.Clamp((50 - c.stats.bloodamount) / 50,
                    0, 1))
            end
    ...
    lockedhands = {
        update = function(c, i)
            ...
            if handcuffed then
                ...
                if leftHandItem ~= nil and leftHandItem ~= handcuffs and leftlockitem == nil then leftHandItem.Drop(c
                    .character) end
                if rightHandItem ~= nil and rightHandItem ~= handcuffs and rightlockitem == nil then rightHandItem.Drop(
                    c.character) end
            end
    ...
    traumaticshock = {
        update = function(c, i)
            local shouldReduce = (c.stats.sedated and c.afflictions.table.strength > 0) or
            c.afflictions.anesthesia.strength > 15
            c.afflictions[i].strength = c.afflictions[i].strength - (0.5 + HF.BoolToNum(shouldReduce, 1.5)) *
            NT.Deltatime
    ...
    tachycardia = {
        update = function(c, i)
            ...
            local hasSymHarmless =
                not NTC.GetSymptomFalse(c.character, i) and c.afflictions.cardiacarrest.strength < 1 and
                c.afflictions.heartremoved.strength < 1 and (NTC.GetSymptom(c.character, i)
    ...
    hyperventilation = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
                not NTC.GetSymptomFalse(c.character, i) and c.afflictions.respiratoryarrest.strength < 1 and
    ...
    dyspnea = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
                not NTC.GetSymptomFalse(c.character, i) and c.afflictions.respiratoryarrest.strength <= 0 and
                (NTC.GetSymptom(c.character, i)
    ...
    sym_cough = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
                not NTC.GetSymptomFalse(c.character, i) and c.afflictions.sym_unconsciousness.strength <= 0 and
                c.afflictions.lungremoved.strength <= 0 and (NTC.GetSymptom(c.character, i)
    ...
    sym_paleskin = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
                ...
                (NTC.GetSymptom(c.character, i) or c.stats.bloodamount < 60 or c.afflictions.bloodpressure.strength < 50)
                , 2)
        end
    },
    sym_lightheadedness = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
                not NTC.GetSymptomFalse(c.character, i) and c.afflictions.sym_unconsciousness.strength <= 0 and
                (NTC.GetSymptom(c.character, i) or c.afflictions.bloodpressure.strength < 60)
                , 2)
        end
    },
    sym_blurredvision = {
        update = function(c, i)
            ...
            not NTC.GetSymptomFalse(c.character, i) and c.afflictions.sym_unconsciousness.strength <= 0 and
            (NTC.GetSymptom(c.character, i)
                or c.afflictions.bloodpressure.strength < 55), 2)
        end
    },
    sym_confusion = {
        update = function(c, i)
            ...
            (NTC.GetSymptom(c.character, i)
                ...
                or c.afflictions.alcoholwithdrawal.strength > 80), 2)
        end
    },
    sym_headache = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and c.afflictions.sym_unconsciousness.strength <= 0 and
            not c.stats.sedated and (NTC.GetSymptom(c.character, i)
                ...
                or c.afflictions.h_fracture.strength > 0), 2)
        end
    },
    sym_legswelling = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and HF.GetAfflictionStrength(c.character, "rl_cyber", 0) < 0.1 and
            (NTC.GetSymptom(c.character, i)
                or c.afflictions.liverdamage.strength > 40 or c.afflictions.kidneydamage.strength > 60
                or c.afflictions.heartdamage.strength > 80), 2)
        end
    },
    sym_weakness = {
        update = function(c, i)
    ...
    sym_vomiting = {
        update = function(c, i)
            ...
            not NTC.GetSymptomFalse(c.character, i) and (NTC.GetSymptom(c.character, i)
                ...
                or c.afflictions.alcoholwithdrawal.strength > 60), 2)
        end
    },
    sym_nausea = {
        update = function(c, i)
    ...
    sym_hematemesis = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and (NTC.GetSymptom(c.character, i)
                or c.afflictions.internalbleeding.strength > 50), 2)
        end
    },
    fever = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and (NTC.GetSymptom(c.character, i)
                or c.afflictions.sepsis.strength > 5 or c.afflictions.alcoholwithdrawal.strength > 90), 2)
        end
    },
    sym_abdomdiscomfort = {
        update = function(c, i)
            ...
            not NTC.GetSymptomFalse(c.character, i) and c.afflictions.sym_unconsciousness.strength <= 0 and
            (NTC.GetSymptom(c.character, i)
                or c.afflictions.liverdamage.strength > 65), 2)
        end
    },
    sym_bloating = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and (NTC.GetSymptom(c.character, i)
                or c.afflictions.liverdamage.strength > 50), 2)
        end
    },
    sym_jaundice = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and (NTC.GetSymptom(c.character, i)
                or c.afflictions.liverdamage.strength > 80), 2)
        end
    },
    sym_sweating = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and (NTC.GetSymptom(c.character, i)
                or c.afflictions.heartattack.strength > 1 or c.stats.withdrawal > 30), 2)
        end
    },
    sym_palpitations = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and c.afflictions.cardiacarrest.strength <= 0 and
            (NTC.GetSymptom(c.character, i)
                or c.afflictions.alkalosis.strength > 20), 2)
        end
    },
    sym_craving = {
        update = function(c, i)
    ...
    forceprone = {
        update = function(c, i)
            c.afflictions[i].strength = HF.BoolToNum(
            not NTC.GetSymptomFalse(c.character, i) and c.afflictions.sym_unconsciousness.strength <= 0 and
            not c.character.IsClimbing and (NTC.GetSymptom(c.character, i)
                or (c.stats.lockleftleg and c.stats.lockrightleg and not c.stats.wheelchaired)), 2)
        end
    },
    onwheelchair = {
        update = function(c, i)
    ...
    pain_abdominal = {
        update = function(c, i)
            ...
            not c.stats.sedated and (NTC.GetSymptom(c.character, i)
                ...
                or c.afflictions.t_arterialcut.strength > 0), 2)
        end
    },
    pain_chest = {
        update = function(c, i)
            ...
            (NTC.GetSymptom(c.character, i)
                ...
                or c.afflictions.t_fracture.strength > 0 or c.afflictions.t_arterialcut.strength > 0), 2)
        end
    },
    luabotomy = {
        update = function(c, i)
...
NT.LimbAfflictions = {
    ...
    bleeding = {
        update = function(c, limbaff, i)
            if (limbaff[i].strength > 0 and math.abs(c.stats.clottingrate - 1) > 0.05) then limbaff[i].strength = limbaff
                [i].strength - (c.stats.clottingrate - 1) * 0.1 * NT.Deltatime end
        end
    },
    burn = {
        max = 200,
        update = function(c, limbaff, i)
            if limbaff[i].strength < 50 then
                ...
                c.stats.healingrate * NT.Deltatime
            end
        end
    },
    acidburn = {
    ...
    lacerations = {
        ...
        update = function(c, limbaff, i)
            limbaff[i].strength = limbaff[i].strength -
            (c.afflictions.immunity.prev / 3000 + HF.Clamp(limbaff.bandaged.strength, 0, 1) * 0.1) * c.stats.healingrate *
            NT.Deltatime
        end
    },
    gunshotwound = {
    ...
    blunttrauma = {
        ...
        update = function(c, limbaff, i)
            limbaff[i].strength = limbaff[i].strength -
            (c.afflictions.immunity.prev / 8000 + HF.Clamp(limbaff.bandaged.strength, 0, 1) * 0.1 + HF.Clamp(limbaff.iced.strength, 0, 1) * 0.33) *
            c.stats.healingrate * NT.Deltatime
        end
    },
    internaldamage = {
    ...
    foreignbody = {
        update = function(c, limbaff, i, type)
            if (limbaff[i].strength < 15) then limbaff[i].strength = limbaff[i].strength -
                0.05 * c.stats.healingrate * NT.Deltatime end
            ...
            HF.Minimum(limbaff.infectedwound.strength, 50) / 1000 + foreignbodycutchance
    ...
    gangrene = {
        update = function(c, limbaff, i, type)
            ...
            if (isExtremity(type)) then
                ...
                if (limbaff[i].strength < 15 and limbaff[i].strength > 0) then limbaff[i].strength = limbaff[i].strength -
                    0.01 * c.stats.healingrate * NT.Deltatime end
                if (c.afflictions.sepsis.strength > 5) then limbaff[i].strength = limbaff[i].strength +
                    HF.BoolToNum(HF.Chance(0.04), 0.5 + c.afflictions.sepsis.strength / 150) *
                    NTConfig.Get("NT_gangrenespeed", 1) * NT.Deltatime end
                if (limbaff.arteriesclamp.strength > 0) then limbaff[i].strength = limbaff[i].strength +
                    HF.BoolToNum(HF.Chance(0.1), 1) * 0.5 * NTConfig.Get("NT_gangrenespeed", 1) * NT.Deltatime end
            end
        end
    },
    pain_extremity = {
...
NT.CharStats = {
    ...
    specificOrganDamageHealMultiplier = { getter = function(c) return NTC.GetMultiplier(c.character,
            "anyspecificorgandamage") + HF.Clamp(c.afflictions.afthiamine.strength, 0, 1) * 4 end },
    neworgandamage = {
        getter = function(c)
            return
    ...
    sedated = { getter = function(c) return c.afflictions.analgesia.strength > 0 or
        c.afflictions.anesthesia.strength > 10 or c.afflictions.drunk.strength > 30 or c.stats.stasis end },
    withdrawal = {
        getter = function(c)
            return math.max(
    ...
    wheelchaired = {
        getter = function(c)
            ...
            if (c.stats.lockleftleg or c.stats.lockrightleg or res) then c.stats.speedmultiplier = c.stats
                .speedmultiplier * 0.5 end
