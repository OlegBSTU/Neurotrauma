    ...
    if(HF.HasAffliction(targetCharacter, organName .. "removed",1) and limbtype == LimbType.Torso and HF.HasAfflictionLimb(targetCharacter,"retractedskin",limbtype,99)) then
        ...
        if damage == 100 then
            ...
            if HF.HasAfflictionLimb(targetCharacter, NTCyb.OrganConfigDatas[organName].cyberAffliction, limbtype) then
                if HF.HasAfflictionLimb(targetCharacter,NTCyb.OrganConfigDatas[organName].cyberAffliction,limbtype,99) then
                    -- cybernetic
                    HF.GiveItemAtCondition(usingCharacter, NTCyb.OrganConfigDatas[organName].tier3Item, HF.Clamp(100-damage, 1, 100))
                else
                    -- augmented
                    ...
                    HF.SpawnItemPlusFunction(NTCyb.OrganConfigDatas[organName].tier2Item,postSpawnFunc,params,parentInventory,inventorySpot)
                end
            else
                -- pure flesh
