...
NTCyb.RepairDamagedElectronics = function(item, usingCharacter, targetCharacter, limb, maxHeal, maxMaterialLoss)
	...
	if not HF.GetSkillRequirementMet(usingCharacter, "electrical", 40) then amountHealed = amountHealed / 2 end
...
NTCyb.RepairMaterialLoss = function(item, usingCharacter, targetCharacter, limb, maxHeal, maxMaterialLoss)
	...
	if not HF.GetSkillRequirementMet(usingCharacter, "mechanical", 60) then amountHealed = amountHealed / 2 end
...
NTCyb.ItemMethods.ekutility_hullrepairkit = function(item, usingCharacter, targetCharacter, limb)
	...
	if containedItem.Condition <= 0 then HF.RemoveItem(containedItem) end
end
...
NTCyb.RepairBentMetal = function(item, usingCharacter, targetCharacter, limb, maxHeal, maxMaterialLoss)
	...
	if not HF.GetSkillRequirementMet(usingCharacter, "mechanical", 50) then amountHealed = amountHealed / 2 end
...
Timer.Wait(function()
	...
	local function removeCyberOrgan(item, usingCharacter, targetCharacter, limb, organConfig)
		...
		local procureready = HF.GetAfflictionStrength(targetCharacter, organConfig.removedAffliction, 0) <= 0
			and HF.GetAfflictionStrength(targetCharacter, organConfig.swapAffliction, 0) >= 0.1
		if not procureready then
			if HF.GetSurgerySkillRequirementMet(usingCharacter, organConfig.surgerySkillRemoval) then
				if HF.GetAfflictionStrength(targetCharacter, organConfig.damageAffliction, 0) >= 100 then
					HF.SetAffliction(targetCharacter, organConfig.removedAffliction, 100, usingCharacter)
				else
					HF.SetAffliction(targetCharacter, organConfig.swapAffliction, 100, usingCharacter)
				end
			else
		...
		elseif HF.HasAfflictionLimb(targetCharacter, organConfig.cyberAffliction, limbtype) then
			...
			if removed <= 0 then
				HF.SetAffliction(targetCharacter, organConfig.removedAffliction, 100, usingCharacter)
				HF.SetAffliction(targetCharacter, organConfig.swapAffliction, 0, usingCharacter)
			end
			...
			if HF.GetSurgerySkillRequirementMet(usingCharacter, organConfig.surgerySkillRemoval) then
				...
				if organConfig.cyberAffliction == "ntc_cyberbrain" then
					...
					if HF.HasAfflictionLimb(targetCharacter, organConfig.cyberAffliction, limbtype, 99) then
						if toContainer then
							HF.SpawnItemPlusFunction(organConfig.tier3Item, nil, nil, container.OwnInventory)
						else
							HF.GiveItemAtCondition(usingCharacter, organConfig.tier3Item, 100)
						end
					else
						if toContainer then
							HF.SpawnItemPlusFunction(organConfig.tier2Item, nil, nil, container.OwnInventory)
						else
				...
				elseif HF.HasAfflictionLimb(targetCharacter, organConfig.cyberAffliction, limbtype, 99) then
					-- cybernetic
					if toContainer then
						HF.SpawnItemPlusFunction(organConfig.tier3Item, postSpawnFunc, params, container.OwnInventory)
					else
					...
					-- augmented
					if toContainer then
						HF.SpawnItemPlusFunction(organConfig.tier2Item, postSpawnFunc, params, container.OwnInventory)
					else
	...
	NT.ItemMethods.organscalpel_kidneys = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTCyb.OrganConfigDatas["kidney"])
	end
	NT.ItemMethods.organscalpel_liver = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTCyb.OrganConfigDatas["liver"])
	end
	NT.ItemMethods.organscalpel_lungs = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTCyb.OrganConfigDatas["lung"])
	end
	NT.ItemMethods.organscalpel_heart = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTCyb.OrganConfigDatas["heart"])
	end
	NT.ItemMethods.organscalpel_brain = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTCyb.OrganConfigDatas["brain"])
	end
